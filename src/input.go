...
func (ib *InputBits) SetInput(in int) {
	if 0 <= in && in < len(sys.keyConfig) {
		*ib = InputBits(
			Btoi(sys.keyConfig[in].U() || sys.joystickConfig[in].U()) |
...
func (ib InputBits) GetInput(cb *CommandBuffer, facing int32) {
	var Back, Forward bool
	if facing < 0 {
		Back, Forward = ib&IB_PR != 0, ib&IB_PL != 0
	} else {
		Back, Forward = ib&IB_PL != 0, ib&IB_PR != 0
	}
	cb.Input(Back, ib&IB_PD != 0, Forward, ib&IB_PU != 0, ib&IB_A != 0, ib&IB_B != 0,
...
func (cl *CommandList) Input(i int, facing int32, aiLevel float32) bool {
	...
	if _else {
		...
		cl.Buffer.Input(Back, D, Forward, U, a, b, c, x, y, z, s, d, w, m)
	}
	return step
}
